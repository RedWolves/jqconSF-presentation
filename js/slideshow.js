jQuery(document).ready(function(){
	...
	jQuery.fn.goto = function(){
		...
		if ( self.length ) {
			...
			jQuery(document.body)
				.animate({scrollTop: self.offset().top}, 1500, "easeOutBounce", function(){
					animating = false;
	...
	$(".dialog").dialog({autoOpen: false, width: 900, height: 725});
	$(".demo").button();
	$("#interactions").click(function(){
		$("#interactions-dialog").dialog("open");
	});
	$("#widgets").click(function(){
		$("#widgets-dialog").dialog("open");
	});
	$("#effects").click(function(){
		$("#effects-dialog").dialog("open");
	});
	$(".showOnClick").hide();
	$(".click").click(function(){
		$(this).fadeOut("slow");
		$(".showOnClick").delay(700).fadeIn("slow");
	})
});